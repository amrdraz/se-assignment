...
describe("getQuotesFromJSON", function() {
    ...
    it("first quote in the array's author should be مزاجنجي", function() {
        assert(quotes[0].author == 'مزاجنجي', "Expected first quote's author to be مزاجنجي" );
    });
});
describe("getQuoteFromJSON", function() {
    it('should return a quote object with an author and text property', function() {
        var res = Quote.getQuoteFromJSON(0);
        assert.isObject(res,"res is a quote Object !!");
        assert.property(res,"author");
        assert.property(res,"text");
    });
    it('should return a random quote if index not specified', function() {
        var res = Quote.getQuoteFromJSON();
        var all = Quote.getQuotesFromJSON();
        assert.include(all, res);
    });
...
describe('seed', function() {
    ...
    it('should populate the db if db is empty returning true', function(done) {
        Quote.seed(function(err, seeded) {
           //if seeded ==true and there is no errors
    ...
    it('should have populated the quotes collection with 102 document', function(done) {
        db.db().collection("quotes").count(function(err, c) {
            //if the number of quotes ==102 and there are no errors while fetching the number of quotes
            assert.equal(null, err);
            assert.equal(102, c);
            done()
        })
    ...
    it('should not seed db again if db is not empty returning false in the callback', function(done) {
        Quote.seed(function(err, seeded) {
            // if the DB already seeded then the seeded flag will be == false and ofcourse there are no errors
            assert.equal(null, err);
