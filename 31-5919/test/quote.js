...
describe("getElementByIndexElseRandom", function() {
    ...
    it("should return the first element if we also pass the index 0", function() {
       expect(Quote.getElementByIndexElseRandom(err, 0)).to.equal(1);
    });
...
describe("getQuotesFromJSON", function() {
    ...
    it("first quote in the array's author should be Kevin Kruse", function() {
        expect((Quote.getQuotesFromJSON()[0]).aurthor).to.equal('Kevin Kruse');
    });
});
...
describe("getQuoteFromJSON", function() {
    ...
    it('should return a random quote if index not specified', function() {
       ...
       var randQuotes=Quote.getQuoteFromJSON();
       assert.include(allQuotes, randQuotes, "quote in array.");
    });
    it('should return the first quote if we pass 0', function() {
        expect(Quote.getQuoteFromJSON(0)).to.equal({"author":"Kevin Kruse", "text":"Life isn’t about getting and having, it’s about giving and being"});
    });
});
...
describe('getQuotesFromDB', function() {
    it('should return all quote documents in the database', function(done) {
        var d = getQuotesFromDB();
        assert.equal(d.length, 102);
